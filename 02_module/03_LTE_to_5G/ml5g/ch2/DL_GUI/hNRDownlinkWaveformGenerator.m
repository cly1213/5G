    ...
    for nch = 1:length(pdsch)
        ...
        for i = 1:length(allocatedSlots)
            ...
            [pdschREindices,dmrsREindices,dmrsSymbols,ptrsREindices,ptrsSymbols,modinfo] = ...
                ...
                'SubcarrierSpacing',bwp(ch.BWP).SubcarrierSpacing,'RBOffset',bwp(ch.BWP).RBOffset),ch);
...
% Get the valid, usable PRB associated with a CORESET definition
% This function returns the PRB associated with the CORESET PRB which overlap
% the AllocatedPRB parameter
function [coresetprb,nblocks] = getCORESETPRB(carrier,bwp,coreset)
    ...
    nblocks = unique(floor((coreset.AllocatedPRB - offsetprb)/6));   % Indices of blocks associated by the CORESET AllocatedPRB parameter
