...
waveconfig = [];
waveconfig.NCellID = @gnb_NCellID@;            % Cell identity
waveconfig.ChannelBandwidth = @gnb_ChannelBandwidth@;  % Channel bandwidth (MHz)
waveconfig.FrequencyRange = 'FR1'; % 'FR1' or 'FR2'
waveconfig.NumSubframes = @gnb_NumSubframes@;      % Number of 1ms subframes in generated waveform (1,2,4,8 slots per 1ms subframe, depending on SCS)
...
carriers(1).SubcarrierSpacing = @gnb_SCSConfig@;
carriers(1).NRB = @gnb_MaxNRB@;
...
bwp(1).SubcarrierSpacing = @gnb_SCSConfig@;          % BWP Subcarrier Spacing
bwp(1).CyclicPrefix = 'Normal';         % BWP Cyclic prefix for 15 kHz
bwp(1).NRB = @bwp_NRB@;                        % Size of BWP
bwp(1).RBOffset = @bwp_NRBOffset@;                   % Position of BWP in SCS carrier
...
coreset = [];
coreset(1).AllocatedSymbols = @coreset_AllocatedSymbols@;    % First symbol of each CORESET monitoring opportunity in a slot
coreset(1).AllocatedSlots = @coreset_AllocatedSlots@;      % Allocated slots within a period
coreset(1).AllocatedPeriod = @coreset_AllocatedPeriod@;         % Allocated slot period (empty implies no repetition)
coreset(1).Duration = @coreset_Duration@;                % CORESET symbol duration (1,2,3)
coreset(1).AllocatedPRB = @coreset_AllocatedPRB@;    % PRB indices, relative to BWP, which select the blocks of 6 RB (RRC - frequencyDomainResources)
...
pdcch(1).AllocatedSearchSpaces = @PDCCH_SearchSpaces@; % Index of allocated search spaces in the CORESET monitoring sequence
pdcch(1).AllocatedPeriod = @PDCCH_AllocatedPeriod@;           % Allocation slot period (empty implies no repetition)
pdcch(1).NumCCE = @PDCCH_NumCCE@;                    % Number of CCE used by PDCCH
pdcch(1).StartCCE = @PDCCH_StartCCE@;                  % Starting CCE of PDCCH
pdcch(1).RNTI = 0;                      % RNTI
...
pdsch(1).TargetCodeRate = @PDSCH_TargetCodeRate@;       % Code rate used to calculate transport block sizes
pdsch(1).Xoh_PDSCH = 0;                 % Rate matching overhead
pdsch(1).Modulation = '@PDSCH_Modulation@';           % 'QPSK', '16QAM', '64QAM', '256QAM'
pdsch(1).NLayers = @PDSCH_NLayers@;                   % Number of PDSCH layers
...
pdsch(1).AllocatedSymbols = @PDSCH_AllocatedSymbols@;      % Range of symbols in a slot
pdsch(1).AllocatedSlots = @PDSCH_AllocatedSlots@;       % Allocated slot indices for PDSCH sequence
pdsch(1).AllocatedPeriod = @PDSCH_AllocatedPeriod@;         % Allocation period in slots (empty implies no repetition)
pdsch(1).AllocatedPRB = @PDSCH_AllocatedPRB@;  % PRB allocation
pdsch(1).RNTI = 0;                     % RNTI
...
pdsch(1).PDSCHMappingType = '@PDSCH_MappingType@';         % PDSCH mapping type ('A'(slot-wise),'B'(non slot-wise))
pdsch(1).DMRSTypeAPosition = @DMRS_DL_DMRS_typeA_pos@;          % Mapping type A only. First DM-RS symbol position (2,3)
pdsch(1).DMRSLength = 1;                 % Number of front-loaded DM-RS symbols (1(single symbol),2(double symbol))   
pdsch(1).DMRSAdditionalPosition = @PDSCH_DL_DMRS_add_pos@;     % Additional DM-RS symbol positions (max range 0...3)
pdsch(1).DMRSConfigurationType = @PDSCH_DL_DMRS_config_type@;      % DM-RS configuration type (1,2)
...
pdsch(1).EnablePTRS = @PDSCH_EnablePTRS@;            % Enable or disable the PT-RS (1 or 0)
