...
function [pxschIndices,dmrsIndices,dmrSymbols,pxschInfo,genInfo] ...
    ...
    if isfield(pxsch,'SymbolSet')
        allocatedsymbols = pxsch.SymbolSet(pxsch.SymbolSet < symbperslot);
    else
    ...
    if ~isempty(allocatedsymbols) && isempty(dmrssymbols)
        ...
        warning('nr5g:DMRSParametersNoSymbols','No DM-RS symbols are defined for the %s. Check DM-RS configuration parameters and %s allocation.',chname,chname);
    end
    ...
    if iscell(pxsch.PRBSet)
        prbcell(allocatedsymbols+1) = cellfun(@(x)reshape(mod(x,gnb.NRB),1,[]),pxsch.PRBSet,'UniformOutput',false);
    else
        prbset = reshape(mod(pxsch.PRBSet,gnb.NRB),1,[]);        % Ensure that it is now a row for implicit expansion later
